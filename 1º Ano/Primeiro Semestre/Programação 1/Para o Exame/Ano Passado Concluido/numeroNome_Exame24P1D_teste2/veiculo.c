...
// Função que permite inserir um novo Veiculo no vetorVeiculos
...
// Função que procura um Veiculo pela matricula.
// A função devolve -1 se não encontrar o Veiculo ou devolve a posição(indice) do Veiculo no vetor
...
// Função que lista no ecra os dados de todos os Veiculos inseridos no vetor
...
// Função que escreve no ecra os dados de um Veiculo
